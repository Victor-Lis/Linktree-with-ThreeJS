...
})({"c4kXa":[function(require,module,exports) {
...
var _linkedinPng = require("./imgs/linkedin.png");
var _linkedinPngDefault = parcelHelpers.interopDefault(_linkedinPng);
var _githubPng = require("./imgs/github.png");
...
const groundGeo = new _three.PlaneGeometry(250, 250);
const groundMat = new _three.MeshStandardMaterial({
    color: 0xffffff,
    side: _three.DoubleSide,
    // wireframe: true,
    map: textureLoader.load((0, _grassSvgDefault.default))
});
const groundMesh = new _three.Mesh(groundGeo, groundMat);
(0, _sceneJs.scene).add(groundMesh);
const groundBody = new _cannonEs.Body({
    //shape: new CANNON.Plane(),
    //mass: 10
    shape: new _cannonEs.Box(new _cannonEs.Vec3(125, 125, 0.1)),
...
// Sphere
const spherePhysMat = new _cannonEs.Material();
const sphereGeo = new _three.SphereGeometry(5);
const sphereMat = new _three.MeshStandardMaterial({
    color: 0xf00000,
    wireframe: true
});
const sphereMesh = new _three.Mesh(sphereGeo, sphereMat);
(0, _sceneJs.scene).add(sphereMesh);
...
const groundSphereContactMat = new _cannonEs.ContactMaterial(groundPhysMat, spherePhysMat, {
    friction: 1,
    restitution: 0
});
...
const baseSphereContactMat = new _cannonEs.ContactMaterial(base.physMat, spherePhysMat, {
    friction: 0.5
});
...
const wall1SphereContactMat = new _cannonEs.ContactMaterial(wall1.physMat, spherePhysMat, {
    friction: 1,
    restitution: 2
});
...
const wall2SphereContactMat = new _cannonEs.ContactMaterial(wall2.physMat, spherePhysMat, {
    friction: 1,
    restitution: 2
});
...
const wall3SphereContactMat = new _cannonEs.ContactMaterial(wall3.physMat, spherePhysMat, {
    friction: 1,
    restitution: 2
});
...
const wall4SphereContactMat = new _cannonEs.ContactMaterial(wall4.physMat, spherePhysMat, {
    friction: 1,
    restitution: 2
});
...
const roofSphereContactMat = new _cannonEs.ContactMaterial(roof.physMat, spherePhysMat, {
    friction: 0.5
});
...
const linkedinBaseWall1SphereContactMat = new _cannonEs.ContactMaterial(linkedinBaseWall1.physMat, spherePhysMat, {
    restitution: 0
});
...
const linkedinBaseWall2SphereContactMat = new _cannonEs.ContactMaterial(linkedinBaseWall2.physMat, spherePhysMat, {
    restitution: 0
});
...
const linkedinRampSphereContactMat = new _cannonEs.ContactMaterial(linkedinRamp.physMat, spherePhysMat, {
    friction: 1000
});
...
const linkedinFloor = createWall({
    ...
    bodyPositionZ: 87.5,
    color: 0xffffff,
    map: (0, _linkedinPngDefault.default)
});
const linkedinFloorSphereContactMat = new _cannonEs.ContactMaterial(linkedinFloor.physMat, spherePhysMat, {
    restitution: 5
});
const point2Light = new _three.PointLight(0x000ff0, 12500, 300);
...
const githubBaseWall1SphereContactMat = new _cannonEs.ContactMaterial(githubBaseWall1.physMat, spherePhysMat, {
    restitution: 0
});
...
const githubBaseWall2SphereContactMat = new _cannonEs.ContactMaterial(githubBaseWall2.physMat, spherePhysMat, {
    restitution: 0
});
...
const githubBaseWall3SphereContactMat = new _cannonEs.ContactMaterial(githubBaseWall3.physMat, spherePhysMat, {
    restitution: 0
});
...
const githubBaseWall4SphereContactMat = new _cannonEs.ContactMaterial(githubBaseWall4.physMat, spherePhysMat, {
    restitution: 0
});
...
const githubBaseWall5SphereContactMat = new _cannonEs.ContactMaterial(githubBaseWall5.physMat, spherePhysMat, {
    restitution: 0
});
...
const githubFloor = createWall({
    ...
    bodyPositionZ: -75,
    color: 0x202020,
    map: (0, _githubPngDefault.default)
});
const githubFloorSphereContactMat = new _cannonEs.ContactMaterial(githubFloor.physMat, spherePhysMat, {
    restitution: 0
});
const point3Light = new _three.PointLight(0xffffff, 12500, 300);
...
(0, _sceneJs.camera).position.set(sphereBody.position.x, sphereBody.position.y + 50, sphereBody.position.z);
function animate() {
    world.step(timeStep);
    ...
    const lookAt = new _three.Vector3(sphereBody.position.x, sphereBody.position.y, sphereBody.position.z);
    (0, _sceneJs.camera).lookAt(lookAt);
    if (sphereMesh.position.z > 53 && sphereMesh.position.x > 53) setTimeout(()=>{
        if (!redirecting) // window.location.href = "https://www.linkedin.com/in/victor-lis-bronzo/"
        redirecting = true;
    }, 1000);
    if (sphereMesh.position.z > -102 && sphereMesh.position.z < -57 && sphereMesh.position.x < -44 && sphereMesh.position.x > -102) setTimeout(()=>{
        if (!redirecting) // window.location.href = "https://www.github.com/Victor-Lis"
        redirecting = true;
    }, 1000);
...
window.addEventListener("keydown", (e)=>{
    if (e.key.toUpperCase() == "A") move({
        direction: 1
    });
    if (e.key.toUpperCase() == "D") move({
        direction: 2
    });
    if (e.key.toUpperCase() == "W") move({
        direction: 3
    });
    if (e.key.toUpperCase() == "S") move({
        direction: 4
    });
});
function move({ direction }) {
    if (direction == 1) sphereBody.angularVelocity.set(10, 0, 0);
    else if (direction == 2) sphereBody.angularVelocity.set(-10, 0, 0);
    else if (direction == 3) sphereBody.angularVelocity.set(0, 0, 10);
    else sphereBody.angularVelocity.set(0, 0, -10);
}
window.addEventListener("resize", function() {
    canvas.style.minWidth = `${window.innerWidth}px`;
    canvas.style.minHeight = `${window.innerHeight}px`;
    (0, _sceneJs.camera).aspect = parseInt(canvas.style.minWidth) / parseInt(canvas.style.minHeight);
    (0, _sceneJs.camera).updateProjectionMatrix();
    (0, _sceneJs.renderer).setSize(parseInt(canvas.style.minWidth), parseInt(canvas.style.minHeight));
});

},{"three":"dfnD0","cannon-es":"hgIpQ","dat.gui":"k3xQk","three/examples/jsm/controls/OrbitControls.js":"7wHNO","./imgs/grass.svg":"2SPXb","./imgs/linkedin.png":"k5EcP","./imgs/github.png":"h03eM","@parcel/transformer-js/src/esmodule-helpers.js":"LKKdx","./scene.js":"dMv0c"}],"hgIpQ":[function(require,module,exports) {
/**
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"LKKdx"}],"2SPXb":[function(require,module,exports) {
module.exports = require("7c8a0ea1cd1eebcd").getBundleURL("lORN7") + "grass.ee5cf136.svg" + "?" + Date.now();

},{"7c8a0ea1cd1eebcd":"j4pdK"}],"j4pdK":[function(require,module,exports) {
"use strict";
var bundleURL = {};
...
},{}],"k5EcP":[function(require,module,exports) {
module.exports = require("5dbf16feaade2774").getBundleURL("lORN7") + "linkedin.cb0cdc1e.png" + "?" + Date.now();

},{"5dbf16feaade2774":"j4pdK"}],"h03eM":[function(require,module,exports) {
module.exports = require("bcfa77e2ebbf505b").getBundleURL("lORN7") + "github.1f775497.png" + "?" + Date.now();

},{"bcfa77e2ebbf505b":"j4pdK"}]},["c4kXa","8ZNvh"], "8ZNvh", "parcelRequirebe84")
